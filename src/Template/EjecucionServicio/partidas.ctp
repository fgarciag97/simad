<?php $this->assign('title', 'Requisicion de Servicio'); ?>
...
<?= $this->Form->create(@$ejecucion, ['class'=>'well']);?>
	<table class="table table-responsive">
		<thead>
			<tr>
				<th></th>
				<th class="text-center"><strong>Requisicion Servicio <?= $req ?></strong></th>
				<th></th>
			</tr>
		</thead>
	</table>
	...
	<?php 
		$beneficiario  = @$concepto['rif'];
		
		if(@$concepto['concepto_mod'] != ''){
			$concepto = @$concepto['concepto_mod'];
		}else{
		...
		<tr>
			<td>
				<div id="" class="input-group">
					<span class="input-group-addon">Beneficiario (opcional)</span>
					<?php if($status['status'] == 1){
						echo $this->Form->select('beneficiario', @$beneficiarios, ['empty' => '--Seleccione--', 'class'=> 'form-control', 'required', 'value' => $beneficiario]);
					}else{
						echo $this->Form->select('beneficiario', @$beneficiarios, ['empty' => '--Seleccione--', 'class'=> 'form-control', 'required', 'readonly', 'value' => $beneficiario]);
					} ?>
				</div>
			</td>
		</tr> 
