...
<div class="well">
	<table class="table table-responsive">
		<thead>
			<tr>
				<th></th>
				<th class="text-center"><strong>Ordenes Aprobadas - Nomina</strong></th>
				<th></th>
			</tr>
		</thead>
	</table>
		...
		<table class="table table-responsive">
			...
			<?php if (isset($data) && !empty($data)): 
		    	foreach($data as $reg){ 
		    		...
		    		if(($reg->status == 5 or $reg->status == 6) and $reg->tipo != 'COMPRA' and $reg->tipo != 'TRANSFERENCIA' and $reg->tipo != 'SERVICIO'){ ?>
				 ...
				 <tr style="text-align: center;">
			    	...
			    	<td>
			    		<?php 
							if($reg->status == 5){
								echo $this->Html->link("<i class='fa fa-check'></i>",array('action' => 'enviarnomina', $reg->id), array('tabindex' => '-1', 'escape' => false, 'class' => 'btn btn-success', 'confirm' => __('Desea enviar a tesoreria la orden # {0}?', $numero_orden_pago)));
		...
		<table class="table table-responsive">
			...
			<?php if (isset($data) && !empty($data)): 
		    	foreach($data as $reg){ 
		    		...
		    		if(($reg->status == 5 or $reg->status == 6) and $reg->tipo == 'COMPRA'){ ?>
...
<table class="table table-responsive">
		<thead>
			<tr>
				<th></th>
				<th class="text-center"><strong>Ordenes Aprobadas - Transferencia</strong></th>
				<th></th>
			</tr>
		</thead>
	</table>
		...
		<table class="table table-responsive">
			...
			<?php if (isset($data) && !empty($data)): 
		    	foreach($data as $reg){ 
		    		...
		    		if(($reg->status == 5 or $reg->status == 6) and $reg->tipo == 'TRANSFERENCIA'){ ?>
				 ...
				 <tr style="text-align: center;">
			    	...
			    	<td>
			    		<?php 
							if($reg->status == 5){
								echo $this->Html->link("<i class='fa fa-check'></i>",array('action' => 'enviartransferencia', $reg->id), array('tabindex' => '-1', 'escape' => false, 'class' => 'btn btn-success', 'confirm' => __('Desea enviar a tesoreria la orden # {0}?', $numero_orden_pago)));
							}else if($reg->status == 6){
								echo $this->Html->link("<i class='fa fa-send'></i>",array('action' => ''), array('tabindex' => '-1', 'escape' => false, 'class' => 'btn btn-info', 'disabled'));
							}
						?>
