<?php $this->assign('title', 'Vacaciones Obreros'); ?>

<?= $this->Form->create('MRRHHVacacionesO', array('class'=>'well', 'type' => 'post', 'url' => array('controller' => 'MRRHHVacacionesO', 'action' => 'index'))) ?>
...
<?php if(isset($data)){ ?>
	...
	<?php if(!$estatus){ ?>
					...
					<table class="table table-responsive">
						<thead>
							<tr>
								...
								<td></td>
								<th class="text-center">
										<strong> Personal Incluido a Vacaciones </strong>
									</th>
								<th></th>
								<th></th>
							</tr>
							        ...
							        <td>
							         	<?php if (@$status == 0){ ?>
								         	<a
									            ...
									            onclick="ver_vacaciones_editar_obreros(this);" class="btn btn-primary btn-sm"><i class='fa fa-pencil'></i>
									        </a>
										<?php } ?>
		...
		<?= $this->element('rrhh/botones_reportes_vaca_obreros_sin_procesar') ?>

	  	<!------------------------------------------------------------------------------->

	  	<?php echo $this->Form->create('MRRHHVacacionesO', array('class'=>'well', 'type' => 'post', 'url' => array('action' => 'procesar'))); ?>
		...
		<!------------------------------------------------------------------------------------------------------->
		    ...
		    <?php if (isset($data2)){ ?>
			    <center>
			    	...
			    	<br>
			    	<?= $this->Form->postLink('Eliminar Vacaciones', ['action' => 'eliminar', @$tps, @$ms, @$qs], ['confirm' => __('Desea eliminar la Vacaciones?'), 'class' => 'btn btn-danger btn-sm']) ?>
			    </center>
			<?php } ?>
			...
			<table class="table table-responsive">
				<thead>
					<tr>
						...
						<td></td>
						<th class="text-center">
								<strong> Personal en Vacaciones </strong>
							</th>
						<th></th>
						<th></th>
					</tr>
			    ...
			    <?php if (isset($db) && !empty($db)){
			    	...
			    	foreach($db as $reg){ ?>
				    	...
				    	<tr style="text-align: center;">
					        ...
					        <td>
								<?= $this->Html->link("<i class='fa fa-file-pdf-o'></i> Planilla ",array('controller' => 'MRRHHPlanillaVacaciones', 'action' => 'Planilla', $reg->id_tipo_personal, $reg->mes, $reg->quincena, $requisicion, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm')); ?>
							</td>
	...
	<table class="table table-responsive" style="width: 80%; position: center; margin-left: 10%; padding: 10px;">
	    	...
	    	<?php foreach($nominas as $reg){ 
		    	...
		    	<tr style="text-align: center;">
			        ...
			        <td> <?= @$status ?> </td>
			        <td> <?php
				        	echo $this->Html->link("<i class='fa fa-file-pdf-o'></i> Reporte Nomina ",array('controller' => 'MRRHHVacacionesO', 'action' => 'ResumenNomina', $reg->mes, $reg->quincena, $requisicion, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm'));
				        	echo $this->Html->link("<i class='fa fa-file-pdf-o'></i> Acumulado ",array('controller' => 'MRRHHVacacionesO', 'action' => 'Acumulado', $reg->mes, $reg->quincena, $requisicion, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm')); 	
