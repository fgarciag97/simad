<?php $this->assign('title', 'Cesta Ticket Obreros'); ?>

<?= $this->Form->create('MRRHHCestaTicketObreros', array('class'=>'well', 'type' => 'post', 'url' => array('controller' => 'MRRHHCestaTicketObreros', 'action' => 'index'))) ?>
		...
		<thead>
			<tr>
				...
				<th class="text-center">
						<strong> Cesta Ticket Obreros </strong>
					</th>
				<th></th>
				<th></th>
			</tr>
...
<?php if(isset($data)){ ?>
	...
	<?php if(!@$estatus){ ?>
						    	...
						    	<tr style="text-align: center;">
							        ...
							        <td>
								        <a
								            ...
								            tp = "<?= $tps; ?>"
								            m = "<?= $ms; ?>"
								            onclick="asignar_ct_obreros(this);" class="btn btn-info btn-sm">+
								        </a>
				...
				<?php if(@$tps == 4){ ?>
				<center>
			    	<?= $this->Html->link("<i class='fa fa-file-pdf-o'></i> Reporte",array('action' => 'ReportesObrerosFijos', @$ms, "PDF"), array('tabindex' => '-1', 'target' => '_blank','escape' => false, 'class' => 'btn btn-default btn-sm')) ?>
			    	<?= $this->Html->link("<i class='fa fa-file-pdf-o'></i> Acumulado",array('action' => 'AcumuladoObrerosFijos', $ms, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm')) ?>
			    	<!--<?= $this->Html->link("<i class='fa fa-file-pdf-o'></i> Nº de Requision: " . $requisicion ,array('action' => 'RequisicionObrerosFijos', $ms, $requisicion, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm')) ?>-->
			    </center>
		    <?php }else if(@$tps == 5){ ?>
		    	<center>
			    	<?= $this->Html->link("<i class='fa fa-file-pdf-o'></i> Reporte",array('action' => 'ReportesObrerosContratados', @$ms, "PDF"), array('tabindex' => '-1', 'target' => '_blank','escape' => false, 'class' => 'btn btn-default btn-sm')) ?>
			    	<?= $this->Html->link("<i class='fa fa-file-pdf-o'></i> Acumulado",array('action' => 'AcumuladoObrerosContratados', $ms, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm')) ?>
			    	<!--<?= $this->Html->link("<i class='fa fa-file-pdf-o'></i> Nº de Requision: " . $requisicion ,array('action' => 'RequisicionObrerosContratados', $ms, $requisicion, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm')) ?>-->
			    </center>
			<?php } ?>

	  	<!------------------------------------------------------------------------------->

	  	<?php echo $this->Form->create('MRRHHCestaTicketObreros', array('class'=>'well', 'type' => 'post', 'url' => array('action' => 'procesar'))); ?>
	...
	<table class="table table-responsive" style="width: 80%; position: center; margin-left: 10%; padding: 10px;">
	    	...
	    	<?php foreach($datas as $reg){ 
		    	...
		    	<tr style="text-align: center;">
			        ...
			        <td> <?= @$status ?> </td>
			        <td> <?php
				        	echo $this->Html->link("<i class='fa fa-file-pdf-o'></i> Reporte CestaTicketObreros ",array('controller' => 'MRRHHCestaTicketObreros', 'action' => 'ResumenCestaTicketObreros', $reg->mes, $requisicion, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm'));
				        	echo $this->Html->link("<i class='fa fa-file-pdf-o'></i> Acumulado ",array('controller' => 'MRRHHCestaTicketObreros', 'action' => 'Acumulado', $reg->mes, $requisicion, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm')); 	
				   
				         ?> 
				    </td>
				    <td>
				    	<?php
				        	echo $this->Html->link("<i class='fa fa-search'></i> Ver",array('controller' => 'MRRHHCestaTicketObreros', 'action' => 'index', $reg->id_tipo_personal, $reg->mes), array('tabindex' => '-1','escape' => false, 'class' => 'btn btn-default btn-sm'));
				        ?> 
				    </td>
		    	</tr>
