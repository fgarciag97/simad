<?php $this->assign('title', 'Ordenamiento de Pago - Transferencia'); ?>
		...
		<?php if (isset($data) && !empty($data)): 
	    	foreach($data as $reg){ 
			 ...
			 <tr style="text-align: center;">
		    	...
		    	<td><?= $reg->tipo ?></td>
		    	<td><label> <?= $reg->fecha_ordenamiento  ?></label></td>
		    	<td><?= number_format($reg->monto, 2, ',', '.') ?></td>
		    	<td>
		    		<?php 
		    			
		    			if($reg->status == 5){
							echo $this->Html->link("<i class='fa fa-file-pdf-o'></i> Reporte",array('action' => 'reportetransferenciacomprometido', $reg->id, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm'));
	    				}else{
							echo $this->Html->link("<i class='fa fa-file-pdf-o'></i> Reporte",array('controller' => 'EjecucionTransferencia', 'action' => 'reporte', $reg->mes_denominacion, $reg->id, 'PDF'), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm'));
	    				}
					?>
		    	</td>
		    	<td>
		    		<?php 
						if($reg->status == 4){
							echo $this->Html->link("<i class='fa fa-exclamation-circle'></i>",array('action' => 'ordenartransferencia', $reg->id), array('tabindex' => '-1', 'escape' => false, 'class' => 'btn btn-info'));
						}else if($reg->status == 5){
							echo $this->Html->link("<i class='fa fa-list'></i>",array('action' => 'ordenartransferencia', $reg->id), array('tabindex' => '-1', 'escape' => false, 'class' => 'btn btn-success'));
						?>

							<a
				         		data-toggle = "modal" 
				         		id = "<?= @$reg->id; ?>" 
				         		fecha = "<?= @date_format($reg->fecha, "Y-m-d"); ?>" 
				         		fecha_ordenamiento = "<?= @date_format($reg->fecha_ordenamiento, "Y-m-d"); ?>" 
				         		numero_orden_pago_ord = "<?= @$reg->numero_orden_pago_ord; ?>" 
					            
					            onclick="numeroordentransferencia(this);" class="btn btn-info"><i class='fa fa-pencil'></i>
					        </a>
