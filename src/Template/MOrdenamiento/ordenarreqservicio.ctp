...
<?= $this->Form->create(@$orden, ['class'=>'well']);?>
		...
		<?php foreach ($registros as $reg) { ?>
			...
			<tr style="text-align: center;">
			    	...
			    	<td>
			    		<?php 
				    		if($data['status'] == 4){
				    			echo $this->Html->Link('Eliminar', ['action' => 'deletereqservicio', $reg->id, $requisicion], ['confirm' => __('Desea eliminar este Items?'), 'class' => 'btn btn-danger btn-xs']);
				    		}else{
				    			echo $this->Html->Link('Eliminar', ['action' => 'deletereqservicio', $reg->id, $requisicion], ['confirm' => __('Desea eliminar este Items?'), 'class' => 'btn btn-danger btn-xs', 'disabled']);
				    		}
			    		?>
			    	</td>
			</tr>
    ...
    <table class="table table-responsive">
    	<tr>
    		<td class="pull-left">					    	
				<?= $this->html->Link('Atrás', ['action' => 'recibidasservicio'], ['class'=>'btn btn-default pull-right']); ?>
			</td>
			<td>
				<?php if($data['status'] == 5): ?>
					<?= $this->html->Link('Revertir', ['action' => 'revertirreqservicio', $req], ['class'=>'btn btn-info pull-right', 'confirm' => __('Seguro desea revertir la requisicion # {0}?', $req)]); ?>
				<?php endif; ?>

				<?php if($data['status'] == 4): ?>
					<?= $this->html->Link('Aprobar', ['action' => 'aprobarreqservicio', $req], ['class'=>'btn btn-success btn-success pull-right', 'confirm' => __('Seguro que desea procesar la requisicion # {0}?', $req)]); ?>
					<?= $this->html->Link('Rechazar', ['action' => 'rechazarreqservicio', $req], ['class'=>'btn btn-danger pull-right', 'confirm' => __('Seguro desea rechazar la requisicion # {0}?', $req)]); ?>
				<?php endif; ?>
