...
<?= $this->Form->create(@$orden, ['class'=>'well']);?>
	...
	<?php if(count($registros) == 0){ ?>
		<table class="table table-responsive">
			<tr>
				<td>Concepto <?= $this->Form->input('concepto', ['type' => 'text', 'label' => false, 'class'=> 'form-control', 'autocomplete' => 'off', 'value' => @$concepto, 'rows' => 2, 'onkeyup' => 'mayus(this);']); ?></td>
			</tr>
		</table>

		<table class="table table-responsive">
			<tr>
				<td>Nº de Factura <?= $this->Form->input('n_factura', ['type' => 'text', 'label' => false, 'class'=> 'form-control', 'autocomplete' => 'off', 'maxlength' => '10']); ?></td>
				<td>Nº de Control <?= $this->Form->input('n_control', ['type' => 'text', 'label' => false, 'class'=> 'form-control', 'autocomplete' => 'off', 'maxlength' => '10']); ?></td>
	...
	<?php }else{ ?>
		<table class="table table-responsive">
			<tr>
				<td>Concepto <?= $this->Form->input('concepto', ['type' => 'text', 'label' => false, 'class'=> 'form-control', 'autocomplete' => 'off', 'value' => @$concepto, 'readonly']); ?></td>
			</tr>
		</table>
		...
		<?php foreach ($registros as $reg) { ?>
			...
			<tr style="text-align: center;">
			    	...
			    	<td>
			    		<?php 
				    		if($data['status'] == 4){
				    			echo $this->Html->Link('Eliminar', ['action' => 'deletenomina', $reg->id, $requisicion], ['confirm' => __('Desea eliminar este Items?'), 'class' => 'btn btn-danger btn-xs']);
				    		}else{
				    			echo $this->Html->Link('Eliminar', ['action' => 'deletenomina', $reg->id, $requisicion], ['confirm' => __('Desea eliminar este Items?'), 'class' => 'btn btn-danger btn-xs', 'disabled']);
				    		}
			    		?>
			    	</td>
			</tr>
    ...
    <table class="table table-responsive">
    	<tr>
    		<td class="pull-left">					    	
				<?= $this->html->Link('Atrás', ['action' => 'recibidasnomina'], ['class'=>'btn btn-default pull-right']); ?>
			</td>
			<td>
				<?php if($data['status'] == 5): ?>
					<?= $this->html->Link('Revertir', ['action' => 'revertirnomina', $req], ['class'=>'btn btn-info pull-right', 'confirm' => __('Seguro desea revertir la requisicion # {0}?', $req)]); ?>
				<?php endif; ?>

				<?php if($data['status'] == 4): ?>
					<?= $this->html->Link('Aprobar', ['action' => 'aprobarnomina', $req], ['class'=>'btn btn-success btn-success pull-right', 'confirm' => __('Seguro que desea procesar la requisicion # {0}?', $req)]); ?>
					<?= $this->html->Link('Rechazar', ['action' => 'rechazarnomina', $req], ['class'=>'btn btn-danger pull-right', 'confirm' => __('Seguro desea rechazar la requisicion # {0}?', $req)]); ?>
				<?php endif; ?>
			</td>
		</tr>
	</table>
