<?php $this->assign('title', 'Ordenamiento de Pago - Nomina'); ?>
		...
		<?php if (isset($data) && !empty($data)): 
	    	foreach($data as $reg){ ?>
			 ...
			 <tr style="text-align: center;">
		    	...
		    	<td><label> <?= $reg->mes_denominacion . " " . $reg->quincena_denominacion  ?></label></td>
		    	<td><?= $reg->tipo ?></td>
		    	...
		    	<td>
		    		<?php 
		    			...
		    			if($reg->status == 5){
							echo $this->Html->link("<i class='fa fa-file-pdf-o'></i> Reporte",array('action' => 'reportenominacomprometido', $reg->id, $reg->numero_orden_pago, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm'));
	    				}else{
							echo $this->Html->link("<i class='fa fa-file-pdf-o'></i> Reporte",array('controller' => 'EjecucionHistorialOrdenes', 'action' => 'reportenomina', $reg->id, $reg->numero_orden_pago, "PDF"), array('tabindex' => '-1','target'=>'_blank','escape' => false, 'class' => 'btn btn-default btn-sm'));
	    				}
					?>
		    	</td>
		    	<td>
		    		<?php 
						if($reg->status == 4){
							echo $this->Html->link("<i class='fa fa-exclamation-circle'></i>",array('action' => 'ordenarnomina', $reg->id), array('tabindex' => '-1', 'escape' => false, 'class' => 'btn btn-info'));
						}else if($reg->status == 5){
							echo $this->Html->link("<i class='fa fa-list'></i>",array('action' => 'ordenarnomina', $reg->id), array('tabindex' => '-1', 'escape' => false, 'class' => 'btn btn-success'));
					?>
							...
							<a
					            ...
					            onclick="numeroordennomina(this);" class="btn btn-info"><i class='fa fa-pencil'></i>
					        </a>
