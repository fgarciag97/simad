...
<?= $this->Form->create(@$ejecucion, ['class'=>'well']);?>
	<table class="table table-responsive">
		<thead>
			<tr>
				<th></th>
				<th class="text-center"><strong>Requisicion Nomina <?= $req ?></strong></th>
				<th></th>
			</tr>
		</thead>
	</table>
	...
	<?php 
		if(@$status[0]['concepto_mod'] != ''){
			$concepto = @$status[0]['concepto_mod'];
		}else{
			$concepto = @$status[0]['concepto'];
		}
	?> 
	...
	<table class="table table-responsive">
		...
		<?php if($status[0]['status'] == 1): ?>
			<tr style="text-align: center;">
				...
				<?= $this->Form->hidden('sector', ['required', 'id' => 'sectorx', 'value' => @$sector]); ?>
				<?= $this->Form->hidden('programa', ['required', 'id' => 'programax', 'value' => @$programa]); ?>
				<?= $this->Form->hidden('actividad', ['required', 'id' => 'actividadx', 'value' => @$actividad]); ?>
    ...
    <table class="table table-responsive">
    	<tr>
			...
			<td>
				...
				<?php if($status[0]['status'] == 2 or $status[0]['status'] == 3): ?>
