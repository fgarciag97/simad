...
class MCompraOrdenesServicioController extends AppController
{
    public function index(){
        ...
        $data = $this->paginate($this->ViewCompraOrdenServicio->find('all')->order(['id' => 'DESC']));
    ...
    public function add(){
        ...
        if ($this->request->is('post')) {
            ...
            $compra = $this->CompraOrdenServicio->newEntity();
            $compra = $this->CompraOrdenServicio->patchEntity($compra, $this->request->getData());
            $compra->requisicion = $requisicion;
            $compra->ano = ANO_FISCAL;
            $compra->fecha = date('Y-m-d');

            if ($this->CompraOrdenServicio->save($compra)) {

                $condicion = ['requisicion' => $requisicion];
                $id = $this->CompraOrdenServicio->find('all')->where($condicion)->last();

                $this->Flash->success(__('Orden de servicio registrada. A continuación registre Items y asocie sus partidas'));
    ...
    public function view($id = null, $req = null){
        ...
        $orden = $this->ViewCompraOrdenServicio->find('all')->where($condicion)->last();
        $this->set(compact('orden'));

        $this->loadModel('ViewCompraOrdenServicioItems');
        $condicion = ['ano' => ANO_FISCAL, 'id_orden' => $id];
        $data = $this->paginate($this->ViewCompraOrdenServicioItems->find('all')->where($condicion));
        $this->set(compact('data'));
        ...
        if ($this->request->is('post')) {
            ...
            $orden = $this->CompraOrdenServicio->find('all', ['conditions' => $condicion])->last()->toArray();

            //========================================================
            //Almacena los datos ingresados
            //========================================================
            $this->loadModel('CompraOrdenServicioItems');
            $compra = $this->CompraOrdenServicioItems->newEntity();
            $compra = $this->CompraOrdenServicioItems->patchEntity($compra, $this->request->getData());
            $compra->id_orden = $orden['id'];
            $compra->fecha = date('Y-m-d');
            $compra->ano = ANO_FISCAL;

            if ($this->CompraOrdenServicioItems->save($compra)) {

                $this->Flash->success(__('Orden de servicio registrada'));
    ...
    public function EditarServicio(){
        ...
        $compra = $this->CompraOrdenServicioItems->get($this->request->data['id']);
    ...
    public function EditarOrden(){
        $this->request->allowMethod(['post', 'get', 'save']);
        $this->loadModel('CompraOrdenServicio');
        $compra = $this->CompraOrdenServicio->get($this->request->data['id']);
        $compra->descripcion_ordena = $this->request->data['descripcion_ordena'];

        if ($this->CompraOrdenServicio->save($compra)) {
            $this->Flash->success(__('Descripcion Actualizado'));
        } else {
            $this->Flash->error(__('Error. Intente de nuevo'));
        }
    ...
    public function delete($id = null)
    {
        ...
        $this->loadModel('CompraOrdenServicioItems');
        $this->CompraOrdenServicioItems->deleteAll(['id_orden IN' => $id]);
        
        $this->loadModel('CompraOrdenServicio');
        $this->request->allowMethod(['post', 'delete']);
        $data = $this->CompraOrdenServicio->get($id);

        if ($this->CompraOrdenServicio->delete($data)) {
    ...
    public function deleteItem($id = null, $req = null, $orden = null)
    {
        $this->loadModel('CompraOrdenServicioItems');
        $this->request->allowMethod(['post', 'delete']);
        $data = $this->CompraOrdenServicioItems->get($id);
        if ($this->CompraOrdenServicioItems->delete($data)) {
    ...
    public function reporte($id = null, $req = null){
        ...
        $pdf -> SetX(70);
        $pdf -> Cell(80,5,'ORDEN DE SERVICIO: ' . substr($numero_orden_pago,6,10),0,1,'C');
        $pdf -> ln();
        $pdf -> ln();
        $pdf -> ln();
        ...
        $pdf -> ln();
        $pdf -> SetX(15);
        $pdf -> MultiCell(180,4, $this->mayus($orden[0]['descripcion_ordena']),0,'J',0);
        $pdf -> SetFont('Arial','',8);
        ...
        $data2 = $this->ViewCompraOrdenServicioItems->find();
        $data2->select([
            'id_partida', 
            'descripcion_partida', 
            'unidad_solicitante', 
