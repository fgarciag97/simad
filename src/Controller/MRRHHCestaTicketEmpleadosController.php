...
class PDF_CT_EMPLEADOS_FIJOS extends \FPDF {
   function Header()
    {
    ...
    $this->SetX(140);
    $this->Cell(80,5,'NOMINA DE: '. 'EMPLEADOS FIJOS'. ' -  BONO DE ALIMENTACION',0,1,'C');
...
class PDF_CT_EMPLEADOS_CONTRATADOS extends \FPDF {
    function Header()
    {
    ...
    $this->Cell(80,5,'REPORTE DE ASIGANCIONES',0,1,'C');
    $this->SetX(140);
    $this->Cell(80,5,'NOMINA DE: '. 'EMPLEADOS CONTRATADOS'. ' -  BONO DE ALIMENTACION',0,1,'C');
...
class PDF_CT_DIRECTIVOS extends \FPDF {
    function Header()
    {
    ...
    $this->Cell(80,5,'REPORTE DE ASIGANCIONES',0,1,'C');
    $this->SetX(140);
    $this->Cell(80,5,'NOMINA DEL: '. 'PERSONAL DIRECTIVO'. ' -  BONO DE ALIMENTACION',0,1,'C');
...
class MRRHHCestaTicketEmpleadosController extends AppController
{
    ...
    public function index($tp = null, $m = null){
        ...
        $tipo_nomina = $this->MrrhhTipoPersonal->find('all')->where(['AND' => ['id >' => 0, 'id <' => 4]])->toArray();
        $options = [];
        ...
        if($tp != null and $m != null ){
            ...
            if($tp == 1){
                ...
                $query2 = $this->ViewMrrhhComisionServicio->find('all');

                $this->loadModel('ViewMrrhhHonorarioP');
                $query3 = $this->ViewMrrhhHonorarioP->find('all');
            }


            $condicion = ['ano' => ANO_FISCAL, 'mes' => $m, 'id_tipo_personal' => $tp];

            $this->loadModel('ViewMrrhhCestaTicketEmpleados');
            $n = $this->ViewMrrhhCestaTicketEmpleados->find('all', ['conditions' => $condicion])->count();
            ...
            if($n == 0){
                ...
                foreach ($query as $reg) {
                    ...
                    $this->loadModel('MrrhhCestaTicketEmpleados');
                    $ct = $this->MrrhhCestaTicketEmpleados->newEntity();
                    $ct = $this->MrrhhCestaTicketEmpleados->patchEntity($ct, $this->request->getData());
                    ...
                    if($ct->cedula != 12732280 and $ct->cedula != 14794432){
                        $this->MrrhhCestaTicketEmpleados->save($ct);
                    }
                }
        ...
        if($tp != '' and $m != ''){
            ...
            $this->loadModel('ViewMrrhhCestaTicketEmpleados');
            $data = $this->paginate($this->ViewMrrhhCestaTicketEmpleados->find('all', ['conditions' => $condicion]));
            $this->set(compact('data'));

            $n = $this->ViewMrrhhCestaTicketEmpleados->find('all', ['conditions' => $condicion]);
            $this->set('n', $n);
            ...
            $this->loadModel('ViewMrrhhCestaTicketEmpleados');
            $db = $this->ViewMrrhhCestaTicketEmpleados->find('all', ['conditions' => $condicion]);
            $this->set(compact('db'));

            $ndb = $this->ViewMrrhhCestaTicketEmpleados->find('all', ['conditions' => $condicion]);
            $this->set('ndb', $ndb);
    ...
    public function procesar(){
        ...
        if ($this->request->is('post')) {
            ...
            if($this->MrrhhRequisicion->save($req)){
                ...
                $this->loadModel('ViewMrrhhCestaTicketEmpleados');
                $query = $this->ViewMrrhhCestaTicketEmpleados->query();
                $query->update()
                    ->set(['procesado' => 1])
    ...
    public function eliminar($tp = null, $m = null){
        ...
        if($tp == 1 or $tp == 2 or $tp == 3){
            ...
            $this->loadModel('MrrhhCestaTicketEmpleados');
            $this->MrrhhCestaTicketEmpleados->deleteAll(['id_tipo_personal IN' => $tp, 'ano IN' => ANO_FISCAL, 'mes IN' => $m]);
        }
    ...
    public function eliminarp($id = null, $tp = null, $m = null){
            ...
            $this->loadModel('MrrhhCestaTicketEmpleados');
            $this->MrrhhCestaTicketEmpleados->deleteAll(['id_tipo_personal IN' => $tp, 'ano IN' => ANO_FISCAL, 'mes IN' => $m, 'id' => $id]);
	...
	public function EditarCT(){
		...
		$query = $this->MrrhhCestaTicketEmpleados->updateAll(['dias_trabajados' => $this->request->data['dias_trabajados'], 'dias_faltados' => $this->request->data['dias_faltados'], 'dias_a_cancelar' => $this->request->data['dias_a_cancelar']], ['id' => $this->request->data['id']]);
	...
	public function ReportesEmpleadosFijos($mes = null){

		$this->loadModel('ViewMrrhhCestaTicketEmpleados');
		$condicion = ['ano' => ANO_FISCAL, 'mes' => $mes, 'id_tipo_personal' => 1];
        $data = $this->ViewMrrhhCestaTicketEmpleados->find('all', ['conditions' => $condicion]);
	    ...
	    $pdf = new PDF_CT_EMPLEADOS_FIJOS('L','mm','LEGAL');
	    $pdf -> AddPage();
	...
	public function ReportesEmpleadosContratados($mes = null){

		$this->loadModel('ViewMrrhhCestaTicketEmpleados');
		$condicion = ['ano' => ANO_FISCAL, 'mes' => $mes, 'id_tipo_personal' => 2];
        $data = $this->ViewMrrhhCestaTicketEmpleados->find('all', ['conditions' => $condicion]);
	...
	public function ReportesDirectivos($mes = null){

		$this->loadModel('ViewMrrhhCestaTicketEmpleados');
		$condicion = ['ano' => ANO_FISCAL, 'mes' => $mes, 'id_tipo_personal' => 3];
        $data = $this->ViewMrrhhCestaTicketEmpleados->find('all', ['conditions' => $condicion]);
    ...
    public function RequisicionEmpleadosFijos($mes = '', $requisicion = ''){
...
',1,'L',0);
    ...
    $pdf -> ln();$pdf -> ln();
    $pdf -> SetX(10);
    $pdf -> MultiCell(200,5,'PAGO DE BONO DE ALIMENTACION DE EMPLEADOS FIJOS CORRESPONDIENTE AL MES DE'.' '. $mes['denominacion'].' '. 'DEL'. ' '. ANO_FISCAL,0,'L',0);
...
',1,'L',0);
    ...
    public function RequisicionEmpleadosContratados($mes = '', $requisicion = ''){
...
',1,'L',0);
    ...
    $pdf -> ln();$pdf -> ln();
    $pdf -> SetX(10);
    $pdf -> MultiCell(200,5,'PAGO DE BONO DE ALIMENTACION DE EMPLEADOS CONTRATADOS CORRESPONDIENTE AL MES DE'.' '. $mes['denominacion'].' '. 'DEL'. ' '. ANO_FISCAL,0,'L',0);
...
',1,'L',0);
    ...
    public function RequisicionDirectivos($mes = '', $requisicion = ''){
...
',1,'L',0);
    ...
    $pdf -> ln();$pdf -> ln();
    $pdf -> SetX(10);
    $pdf -> MultiCell(200,5,'PAGO DE BONO DE ALIMENTACION AL PERSONAL DIRECTIVO CORRESPONDIENTE AL MES DE'.' '. $mes['denominacion'].' '. 'DEL'. ' '. ANO_FISCAL,0,'L',0);
...
',1,'L',0);
    ...
    public function AcumuladoEmpleadosFijos($mes = ''){
        ...
        $pdf -> Cell(95,4,'Bono Compensatorio de Alimentacion',0,0);
        $pdf -> Cell(22,4,'4.01.04.08.00',0,0);

        /* Calculo de totales por accion especifica*/
        $pdf -> Cell(22,4, $this->moneda($t_neto),0,0,'R');
        $pdf -> Cell(22,4,'',0,0);
        $pdf -> Cell(22,4,'',0,1);
        ...
        $pdf -> SetFont('Arial','B',8);
        $pdf -> Cell(22,3, $this->moneda($t_neto),1,0,'R');
        ...
        $pdf->SetX(110);
        $pdf -> Cell(30,3,'Total Empleados ',0,0);
...
',1,'L',0);
    ...
    public function AcumuladoEmpleadosContratados($mes = ''){
       ...
       $i = 0;
        ...
        $pdf -> Cell(10,4,'1001',0,0);
        $pdf -> Cell(95,4,'Bono Compensatorio de Alimentacion',0,0);
        $pdf -> Cell(22,4,'4.01.04.26.00',0,0);
...
',1,'L',0);
    ...
    public function AcumuladoDirectivos($mes = ''){
        ...
        $pdf -> SetX(5);
        $pdf -> Cell(10,4,'1001',0,0);
        $pdf -> Cell(95,4,'Bono Compesatorio de Alimentacion personal de Alto Nivel',0,0);
        $pdf -> Cell(22,4,'4.01.04.46.00',0,0);

        /* Calculo de totales por accion especifica*/
        $pdf -> Cell(22,4, $this->moneda($total),0,0,'R');
        $pdf -> Cell(22,4,'',0,0);
        $pdf -> Cell(22,4,'',0,1);

        $pdf -> SetX(5);
        $pdf -> Cell(10,4,'1002',0,0);
        $pdf -> Cell(95,4,'Bono Compesatorio de Alimentacion personal Altos Funcionarios',0,0);
        $pdf -> Cell(22,4,'4.01.04.51.00',0,0);

        /* Calculo de totales por accion especifica*/
        $pdf -> Cell(22,4, $this->moneda($altos),0,0,'R');
        $pdf -> Cell(22,4,'',0,0);
        $pdf -> Cell(22,4,'',0,1);
...
',1,'L',0);
    ...
    public function txt($tp, $mes, $requisicion){
        ...
        if($tp == 1 or $tp == 2 or $tp == 3){
            $this->loadModel('ViewMrrhhCestaTicketEmpleados');
            $data = $this->ViewMrrhhCestaTicketEmpleados->find('all')->where(['id_tipo_personal' => $tp, 'ano' => ANO_FISCAL, 'mes' => $mes, 'requisicion' => $requisicion]);
        }
    ...
    public function excel($tp, $mes, $requisicion){
        ...
        if($tp == 1){
            ...
            $data2 = $this->ViewMrrhhHistoricoEmpleadosFijos->find('all')->where(['id_tipo_personal' => $tp, 'ano' => ANO_FISCAL, 'mes' => $mes,  'requisicion' => $requisicion]);
        }else if($tp == 2){
            $this->loadModel('ViewMrrhhHistoricoEmpleadosC');
            $data2 = $this->ViewMrrhhHistoricoEmpleadosC->find('all')->where(['id_tipo_personal' => $tp, 'ano' => ANO_FISCAL, 'mes' => $mes,  'requisicion' => $requisicion]);
        }else if($tp == 3){
            $this->loadModel('ViewMrrhhHistoricoDirectivos');
            $data2 = $this->ViewMrrhhHistoricoDirectivos->find('all')->where(['id_tipo_personal' => $tp, 'ano' => ANO_FISCAL, 'mes' => $mes,  'requisicion' => $requisicion]);
            $this->loadModel('ViewMrrhhEmpleadosFijos');
