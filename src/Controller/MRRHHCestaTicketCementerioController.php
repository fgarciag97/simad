...
class PDF_CT_CEMENTERIO extends \FPDF {
    function Header()
    {
    ...
    $this->SetX(140);
    $this->Cell(80,5,'NOMINA DE: '. 'CEMENTERIO'. ' -  BONO DE ALIMENTACION',0,1,'C');
...
class MRRHHCestaTicketCementerioController extends AppController
{
    public function index($tp = null, $m = null){
        ...
        $tipo_nomina = $this->MrrhhTipoPersonal->find('all', ['conditions' => ['tipo_nomina' => 6, 'id' => 15]])->toArray();
        $options = [];
        ...
        if($tp != null and $m != null ){
            ...
            $query = $this->ViewMrrhhCementerio->find('all');   


            $condicion = ['ano' => ANO_FISCAL, 'mes' => $m, 'id_tipo_personal' => $tp];

            $this->loadModel('ViewMrrhhCestaTicketCementerio');
            $n = $this->ViewMrrhhCestaTicketCementerio->find('all', ['conditions' => $condicion])->count();
            ...
            if($n == 0){
                ...
                foreach ($query as $reg) {
                    ...
                    $this->loadModel('MrrhhCestaTicketCementerio');
                    $ct = $this->MrrhhCestaTicketCementerio->newEntity();
                    $ct = $this->MrrhhCestaTicketCementerio->patchEntity($ct, $this->request->getData());
                    ...
                    if($ct->cedula != 12732280 and $ct->cedula != 14794432){
                        $this->MrrhhCestaTicketCementerio->save($ct);
                    }
                }
            }
        }
        ...
        if($tp != '' and $m != ''){
            ...
            $this->loadModel('ViewMrrhhCestaTicketCementerio');
            $data = $this->paginate($this->ViewMrrhhCestaTicketCementerio->find('all', ['conditions' => $condicion]));
            $this->set(compact('data'));

            $n = $this->ViewMrrhhCestaTicketCementerio->find('all', ['conditions' => $condicion]);
            $this->set('n', $n);
            ...
            $this->loadModel('ViewMrrhhCestaTicketCementerio');
            $db = $this->ViewMrrhhCestaTicketCementerio->find('all', ['conditions' => $condicion]);
            $this->set(compact('db'));

            $ndb = $this->ViewMrrhhCestaTicketCementerio->find('all', ['conditions' => $condicion]);
            $this->set('ndb', $ndb);
    ...
    public function procesar(){
        ...
        if ($this->request->is('post')) {
            ...
            if($this->MrrhhRequisicion->save($req)){
                ...
                $this->loadModel('ViewMrrhhCestaTicketCementerio');
                $query = $this->ViewMrrhhCestaTicketCementerio->query();
                $query->update()
                    ->set(['procesado' => 1])
    ...
    public function eliminar($tp = null, $m = null){
        ...
        if($tp == 15){
            ...
            $this->loadModel('MrrhhCestaTicketCementerio');
            $this->MrrhhCestaTicketCementerio->deleteAll(['id_tipo_personal IN' => $tp, 'ano IN' => ANO_FISCAL, 'mes IN' => $m]);
        }
    ...
    public function eliminarp($id = null, $tp = null, $m = null){
            ...
            $this->loadModel('MrrhhCestaTicketCementerio');
            $this->MrrhhCestaTicketCementerio->deleteAll(['id_tipo_personal IN' => $tp, 'ano IN' => ANO_FISCAL, 'mes IN' => $m, 'id' => $id]);
    ...
    public function EditarCT(){
        ...
        $query = $this->MrrhhCestaTicketCementerio->updateAll(['dias_trabajados' => $this->request->data['dias_trabajados'], 'dias_faltados' => $this->request->data['dias_faltados'], 'dias_a_cancelar' => $this->request->data['dias_a_cancelar']], ['id' => $this->request->data['id']]);
    ...
    public function ReportesCementerio($mes = null){

        $this->loadModel('ViewMrrhhCestaTicketCementerio');
        $condicion = ['ano' => ANO_FISCAL, 'mes' => $mes, 'id_tipo_personal' => 15];
        $data = $this->ViewMrrhhCestaTicketCementerio->find('all', ['conditions' => $condicion]);
        ...
        $pdf = new PDF_CT_CEMENTERIO('L','mm','LEGAL');
        $pdf -> AddPage();
        
        $pdf -> SetFont('Arial','I',8);
                $pdf -> SetY(20);
                $pdf->SetX(110);
                $pdf -> MultiCell(140,5,'Nomina: CEMENTERIO (Correspondiente al'.' mes de'.' '.$mes['denominacion'].' '.'del'.' '. ANO_FISCAL,0,'C',0);
                $pdf -> SetX(160);
    ...
    public function RequisicionCementerio($mes = '', $requisicion = ''){
...
',1,'L',0);
    ...
    $pdf -> ln();$pdf -> ln();
    $pdf -> SetX(10);
    $pdf -> MultiCell(200,5,'PAGO DE BONO DE ALIMENTACION AL PERSONAL DE CEMENTERIO CORRESPONDIENTE AL MES DE'.' '. $mes['denominacion'].' '. 'DEL'. ' '. ANO_FISCAL,0,'L',0);
...
',1,'L',0);
    ...
    public function AcumuladoCementerio($mes = '', $requisicion = ''){
        ...
        $pdf -> Cell(120,4,'SEGURIDAD SOCIAL A Cementerio',0,0);
     ...
     $i = 0;
        ...
        $pdf -> SetX(110);
        $pdf -> Cell(33,3,'Cantidad de Empleados',0,0);
...
',1,'L',0);
    ...
    public function txt($tp, $mes, $requisicion){
        ...
        if($tp == 15){
            $this->loadModel('ViewMrrhhCestaTicketCementerio');
            $data = $this->ViewMrrhhCestaTicketCementerio->find('all')->where(['id_tipo_personal' => $tp, 'ano' => ANO_FISCAL, 'mes' => $mes, 'requisicion' => $requisicion]);
        }
