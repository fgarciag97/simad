...
class PDF_HE_EMPLEADOS_FIJOS extends \FPDF {
    function Header()
    {
    ...
    $this->SetX(140);
    $this->Cell(80,5,'HORAS EXTRAS NOMINA DE EMPLEADOS FIJOS',0,1,'C');
...
class PDF_HE_EMPLEADOS_CONTRATADOS extends \FPDF {
    function Header()
    {
    $this->SetFont('Arial','B',10);
    ...
    $this->Cell(80,5,'REPORTE DE ASIGANCIONES',0,1,'C');
    $this->SetX(140);
    $this->Cell(80,5,'HORAS EXTRAS NOMINA DE EMPLEADOS CONTRATADOS',0,1,'C');
...
class PDF_HE_DIRECTIVOS extends \FPDF {
    function Header()
    {
    $this->SetFont('Arial','B',10);
    ...
    $this->Cell(80,5,'REPORTE DE ASIGANCIONES',0,1,'C');
    $this->SetX(140);
    $this->Cell(80,5,'HORAS EXTRAS NOMINA PERSONAL DIRECTIVO',0,1,'C');
    $this->SetX(140);
    $this-> Cell(80,5,'Pago de Horas Extras Diurnas, Horas Extras Nocturnas y Dias No Laborables',0,1,'C');
...
class PDF_ResumenNomina extends \FPDF {
    function Header()
    {
...
class MRRHHHorasExtrasEmpleadosController extends AppController
{
	public function index($tp = null, $m = null, $o = null){
        ...
        $tipo_nomina = $this->MrrhhTipoPersonal->find('all', ['conditions' => ['tipo_nomina' => 1]])->toArray();
        $options = [];
        ...
        if($tp != null and $m != null  and $o != null){
            ...
            if($tp == 1){
                ...
                $query = $this->ViewMrrhhEmpleadosFijos->find('all');

            }else if($tp == 2){
                $this->loadModel('ViewMrrhhEmpleadosContratados');
                $query = $this->ViewMrrhhEmpleadosContratados->find('all');
                
            }else if($tp == 3){
                $this->loadModel('ViewMrrhhDirectivos');
                $query = $this->ViewMrrhhDirectivos->find('all');
            }
            ...
            $n = $this->ViewMrrhhHorasExtrasEmpleados->find('all', ['conditions' => $condicion])->count();
            ...
            if($n == 0){
                ...
                foreach ($query as $reg) {
                    ...
                    $this->loadModel('MrrhhHorasExtrasEmpleados');
                    $he = $this->MrrhhHorasExtrasEmpleados->newEntity();
                    $he = $this->MrrhhHorasExtrasEmpleados->patchEntity($he, $this->request->getData());
                    ...
                    if($he->cedula != 12732280 and $he->cedula != 14794432){
                        $this->MrrhhHorasExtrasEmpleados->save($he);
                    }
                }
            }
        }
        ...
        if($tp != '' and $m != '' and $o != ''){
            ...
            $this->loadModel('ViewMrrhhHorasExtrasEmpleados');
            $data = $this->paginate($this->ViewMrrhhHorasExtrasEmpleados->find('all', ['conditions' => $condicion]));
            $this->set(compact('data'));

            $n = $this->ViewMrrhhHorasExtrasEmpleados->find('all', ['conditions' => $condicion]);
            $this->set('n', $n);
            ...
            $this->loadModel('ViewMrrhhHorasExtrasEmpleados');
            $db = $this->ViewMrrhhHorasExtrasEmpleados->find('all', ['conditions' => $condicion]);
            $this->set(compact('db'));

            $ndb = $this->ViewMrrhhHorasExtrasEmpleados->find('all', ['conditions' => $condicion]);
            $this->set('ndb', $ndb);
    ...
    public function procesar(){
        ...
        if ($this->request->is('post')) {
            ...
            if($this->MrrhhRequisicion->save($req)){
                ...
                $this->loadModel('ViewMrrhhHorasExtrasEmpleados');
                $query = $this->ViewMrrhhHorasExtrasEmpleados->query();
                $query->update()
                    ->set(['procesado' => 1])
    ...
    public function eliminar($tp = null, $m = null, $o = null){
        ...
        if($tp == 1 or $tp == 2 or $tp == 3){
            ...
            $this->loadModel('MrrhhHorasExtrasEmpleados');
            $this->MrrhhHorasExtrasEmpleados->deleteAll(['id_tipo_personal IN' => $tp, 'ano IN' => ANO_FISCAL, 'mes IN' => $m,  'omitida' => $o]);
        }
	...
	public function ReportesEmpleadosFijos($mes = null, $omitida = null){

		$this->loadModel('ViewMrrhhHorasExtrasEmpleados');
		$condicion = ['ano' => ANO_FISCAL, 'mes' => $mes, 'id_tipo_personal' => 1, 'omitida' => $omitida];
        $data = $this->ViewMrrhhHorasExtrasEmpleados->find('all', ['conditions' => $condicion]);
	    ...
	    $pdf = new PDF_HE_EMPLEADOS_FIJOS('L','mm',array(350,250));
	    $pdf -> AddPage();
        ...
        $pdf -> SetX(160);
        $pdf -> SetFont('Arial','',8);
        $pdf -> Cell(40,5,'Nomina: EMPLEADOS FIJOS (Pago de Horas Extras Diurnas, Horas Extras Nocturnas y Dias No Laborables Correspondiente al mes de'.' '.$mes['denominacion'].' '.'del'.' '. ANO_FISCAL,0,1,'C');
        $pdf -> SetX(160);
        ...
        foreach ($data as $key) {
                  ...
                  $asignación = $sueldo_base + $prima_hijos + $prima_hogar + $prima_antiguedad + $prima_profesion;
                  /*====================== Horas Diurnas ===================*/

                  if($key['cantidad_horas_extra_diurnas'] != 0)
                  $diurnas = (((($asignación) / 30) / 8) * 1.70) * $key['cantidad_horas_extra_diurnas'];
                  else $diurnas = 0;

                  /*====================== Horas Nocturnas ===================*/

                  if($key['cantidad_horas_extra_nocturnas'] != 0)
                  $nocturnas = ((((($asignación) / 30) / 8) * 1.70) + (((($asignación) / 30) / 8) * 1.30)) * $key['cantidad_horas_extra_nocturnas'];
                  else $nocturnas = 0;

                  /*====================== Dias no Laborables ===================*/

                  if($key['dias_no_laborables'] != 0)
                  $dias_no_laborables = ((($asignación) / 30) * 1.50) * $key['dias_no_laborables'];
            ...
            if($key['cantidad_horas_extra_diurnas'] != 0 or $key['cantidad_horas_extra_diurnas'] != 0 or $key['dias_no_laborables'] != 0){
                ...
                $pdf -> Cell(25,7, $key['cedula'],1,0,'C');
                $pdf -> Cell(40,7, $this->mayus($key['apellidos']),1,0,'C');
                $pdf -> Cell(40,7, $this->mayus($key['nombres']),1,0,'C');
                $pdf -> Cell(80,7, $this->mayus($key['cargo_fijo']),1,0,'C');
                $pdf -> Cell(20,7,$this->moneda(@$asignación),1,0,'R');
                $pdf -> Cell(10,7, $key['cantidad_horas_extra_diurnas'],1,0,'C');
                $pdf -> Cell(25,7, $this->moneda(@$diurnas),1,0,'R');
                $pdf -> Cell(10,7, $key['cantidad_horas_extra_nocturnas'],1,0,'C');
                $pdf -> Cell(25,7, $this->moneda(@$nocturnas),1,0,'R');
                $pdf -> Cell(10,7, $key['dias_no_laborables'],1,0,'C');
	...
	public function ReportesEmpleadosContratados($mes = null, $omitida = null){

		$this->loadModel('ViewMrrhhHorasExtrasEmpleados');
		$condicion = ['ano' => ANO_FISCAL, 'mes' => $mes, 'id_tipo_personal' => 2, 'omitida' => $omitida];
        $data = $this->ViewMrrhhHorasExtrasEmpleados->find('all', ['conditions' => $condicion]);

        $this->loadModel('ViewMrrhhHistoricoEmpleadosC');
        $condicion = ['ano' => ANO_FISCAL, 'mes' => $mes, 'id_tipo_personal' => 2, 'omitida' => $omitida];
        $data2 = $this->ViewMrrhhHistoricoEmpleadosC->find('all', ['conditions' => $condicion]);
        ...
        $pdf = new PDF_HE_EMPLEADOS_CONTRATADOS('L','mm',array(350,250));
        $pdf -> AddPage();
        ...
        $pdf -> SetX(160);
        $pdf -> SetFont('Arial','',8);
        $pdf -> Cell(40,5,'Nomina: EMPLEADOS CONTRATADOS (Pago de Horas Extras Diurnas, Horas Extras Nocturnas y Dias No Laborables Correspondiente al mes de'.' '.$mes['denominacion'].' '.'del'.' '. ANO_FISCAL,0,1,'C');
        $pdf -> SetX(160);
        $pdf -> Cell(40,5,'Correspondiente al lapso Desde:' . '  ' .$desde. ANO_FISCAL.' '.'Hasta:'.' '.$Hasta. ANO_FISCAL,0,1,'C');
        $pdf->ln();
        $pdf->ln();
        ...
        foreach ($data as $key) {
            ...
            if($key['cantidad_horas_extra_diurnas'] != 0 or $key['cantidad_horas_extra_diurnas'] != 0 or $key['dias_no_laborables'] != 0){
                ...
                $pdf -> Cell(40,7, $this->mayus($key['apellidos']),1,0,'C');
                $pdf -> Cell(40,7, $this->mayus($key['nombres']),1,0,'C');
                $pdf -> Cell(80,7, $this->mayus($cargo),1,0,'C');
                $pdf -> Cell(20,7, $this->moneda(@$asignación),1,0,'R');
                $pdf -> Cell(10,7, $key['cantidad_horas_extra_diurnas'],1,0,'C');
                $pdf -> Cell(25,7, $this->moneda(@$diurnas),1,0,'R');
                $pdf -> Cell(10,7, $key['cantidad_horas_extra_nocturnas'],1,0,'C');
	...
	public function ReportesDirectivos($mes = null, $omitida = null){

		$this->loadModel('ViewMrrhhHorasExtrasEmpleados');
		$condicion = ['ano' => ANO_FISCAL, 'mes' => $mes, 'id_tipo_personal' => 3, 'omitida' => $omitida];
        $data = $this->ViewMrrhhHorasExtrasEmpleados->find('all', ['conditions' => $condicion]);

        $this->loadModel('ViewMrrhhHistoricoDirectivos');
        $data2 = $this->ViewMrrhhHistoricoDirectivos->find('all');
        ...
        $pdf = new PDF_HE_DIRECTIVOS('L','mm',array(350,250));
        $pdf -> AddPage();
        ...
        $pdf -> SetX(160);
        $pdf -> SetFont('Arial','',8);
        $pdf -> Cell(40,5,'Nomina: PERSONAL DIRECTIVO (Pago de Horas Extras Diurnas, Horas Extras Nocturnas y Dias No Laborables Correspondiente al mes de'.' '.$mes['denominacion'].' '.'del'.' '. ANO_FISCAL,0,1,'C');
        $pdf -> SetX(160);
        $pdf -> Cell(40,5,'Correspondiente al lapso Desde:' . '  ' .$desde. ANO_FISCAL.' '.'Hasta:'.' '.$Hasta. ANO_FISCAL,0,1,'C');
        $pdf->ln();
        $pdf->ln();
    ...
    public function RequisicionEmpleadosFijos($mes = '', $omitida = null, $requisicion = null){
...
',1,'L',0);
    ...
    $pdf -> ln();$pdf -> ln();
    $pdf -> SetX(10);
    $pdf -> MultiCell(200,5,'PAGO DE HORAS EXTRAS DIURNAS, HORAS EXTRAS NOCTURNAS Y DIAS NO LABORABLES DE EMPLEADOS FIJOS CORRESPONDIENTE AL MES DE'.' '. $mes['denominacion'].' '. 'DEL'. ' '. ANO_FISCAL,0,'L',0);
...
',1,'L',0);
    ...
    public function RequisicionEmpleadosContratados($mes = '', $omitida = null, $requisicion = null){
...
',1,'L',0);
    ...
    $pdf -> ln();$pdf -> ln();
    $pdf -> SetX(10);
    $pdf -> MultiCell(200,5,'PAGO DE HORAS EXTRAS DIURNAS, HORAS EXTRAS NOCTURNAS Y DIAS NO LABORABLES DE EMPLEADOS CONTRATADOS CORRESPONDIENTE AL MES DE'.' '. $mes['denominacion'].' '. 'DEL'. ' '. ANO_FISCAL,0,'L',0);
...
',1,'L',0);
    ...
    public function RequisicionDirectivos($mes = '', $omitida = null, $requisicion = null){
...
',1,'L',0);
    ...
    $pdf -> ln();$pdf -> ln();
    $pdf -> SetX(10);
    $pdf -> MultiCell(200,5,'PAGO DE HORAS EXTRAS DIURNAS, HORAS EXTRAS NOCTURNAS Y DIAS NO LABORABLES AL PERSONAL DIRECTIVO CORRESPONDIENTE AL MES DE'.' '. $mes['denominacion'].' '. 'DEL'. ' '. ANO_FISCAL,0,'L',0);
...
',1,'L',0);
    ...
    public function AcumuladoEmpleadosFijos($mes = null, $omitida = null){
        ...
        $data2 = $this->ViewMrrhhHistoricoEmpleadosFijos->find('all');
        ...
        $pdf -> SetFont('Arial','I',8);
                $pdf -> SetY(20);
                $pdf->SetX(40);
                $pdf -> MultiCell(140,5,'Nomina: EMPLEADOS FIJOS (Correspondiente al'.' mes de'.' '.$mes['denominacion'].' '.'del'.' '. ANO_FISCAL,0,'C',0);
                $pdf -> SetX(90);
                $pdf -> Cell(40,5,'Correspondiente al lapso Desde:' . '  ' .$desde. ANO_FISCAL.' '.'Hasta:'.' '.$Hasta. ANO_FISCAL,0,1,'C');  
        ...
        if($t_diurnas != 0){
            $pdf -> SetX(5);
            $pdf -> Cell(10,4,'1001',1,0);
            $pdf -> Cell(95,4,'Pago de Horas Extras Dirunas',1,0);
            $pdf -> Cell(22,4,'4.01.03.08.00',1,0);

            /* Calculo de totales por accion especifica*/
            $pdf -> Cell(22,4, $this->moneda($t_diurnas),1,0,'R');
            $pdf -> Cell(22,4,'',1,0);
            $pdf -> Cell(22,4,'',1,1);
        }
        ...
        if($t_nocturnas != 0){
            ...
            $pdf -> Cell(22,4,'4.01.03.04.00',1,0);

            /* Calculo de totales por accion especifica*/
            $pdf -> Cell(22,4, $this->moneda($t_nocturnas),1,0,'R');
            $pdf -> Cell(22,4,'',1,0);
            $pdf -> Cell(22,4,'',1,1);
        }
        ...
        if($t_dias != 0){
            $pdf -> SetX(5);
            $pdf -> Cell(10,4,'1004',1,0);
            $pdf -> Cell(95,4,'Pago por Dias No Laborables ',1,0);
            $pdf -> Cell(22,4,'4.01.03.09.00',1,0);

            /* Calculo de totales por accion especifica*/
            $pdf -> Cell(22,4, $this->moneda($t_dias),1,0,'R');
            $pdf -> Cell(22,4,'',1,0);
            $pdf -> Cell(22,4,'',1,1);
        }
        ...
        $pdf -> SetX(110);
        $pdf -> Cell(30,3,'Cantidad de Empleados',0,0);
...
',1,'L',0); 
    ...
    public function AcumuladoEmpleadosContratados($mes = '', $omitida = null){
        ...
        $this->loadModel('ViewMrrhhHistoricoEmpleadosC');
        $data2 = $this->ViewMrrhhHistoricoEmpleadosC->find('all');
        ...
        $pdf -> SetFont('Arial','I',8);
                $pdf -> SetY(20);
                $pdf->SetX(40);
                $pdf -> MultiCell(140,5,'Nomina: EMPLEADOS CONTRATADOS (Correspondiente al'.' mes de'.' '.$mes['denominacion'].' '.'del'.' '. ANO_FISCAL,0,'C',0);
                $pdf -> SetX(90);
                $pdf -> Cell(40,5,'Correspondiente al lapso Desde:' . '  ' .$desde. ANO_FISCAL.' '.'Hasta:'.' '.$Hasta. ANO_FISCAL,0,1,'C');  
...
',1,'L',0);
    ...
    public function AcumuladoDirectivos($mes = '', $omitida = null){
        ...
        $pdf -> SetFont('Arial','I',8);
                $pdf -> SetY(20);
                $pdf->SetX(40);
                $pdf -> MultiCell(140,5,'Nomina: PERSONAL DIRECTIVO (Correspondiente al'.' mes de'.' '.$mes['denominacion'].' '.'del'.' '. ANO_FISCAL,0,'C',0);
                $pdf -> SetX(90);
                $pdf -> Cell(40,5,'Correspondiente al lapso Desde:' . '  ' .$desde. ANO_FISCAL.' '.'Hasta:'.' '.$Hasta. ANO_FISCAL,0,1,'C');  
        ...
        if($t_diurnas != 0){
            ...
            /* Calculo de totales por accion especifica*/
            $pdf -> Cell(22,4, $this->moneda($t_diurnas),1,'R',0);
            $pdf -> Cell(22,4,'',1,0);
            $pdf -> Cell(22,4,'',1,1);
        }
...
',1,'L',0); 
    ...
    public function ResumenNominaEmpleadosFijos($mes = '', $omitida = ''){
        ...
        $pdf -> SetY(20);
        $pdf->SetX(40);
                $pdf -> MultiCell(140,5,'Nomina: EMPLEADOS FIJOS (Pago de Horas Extras Diurnas, Horas Extras Nocturnas y Dias No Laborables Correspondiente al mes de'.' '.$mes['denominacion'].' '.'del'.' '. ANO_FISCAL,0,'C',0);
                $pdf -> SetX(90);
                $pdf -> Cell(40,5,'Correspondiente al lapso Desde:' . '  ' .$desde. ANO_FISCAL.' '.'Hasta:'.' '.$Hasta. ANO_FISCAL,0,1,'C');
    ...
    if($diurnas != 0 or $nocturnas != 0 or $dias_no_laborables != 0){
            ...
            $pdf -> Cell(50,4,'Tipo Personal: '. $this->mayus($key2['grupo'].' - '.$key2['denominacion_grado_nivel']),1,0,'L');
            if($key['sueldo'] != 0){
            ...
            $pdf -> SetX(5);
            $pdf -> Cell(70,4,'Cargo: '. $this->mayus(substr(substr($key['cargo_fijo'],0,20),0,80)),1,0,'L');
...
',1,'L',0);
    ...
    public function ResumenNominaEmpleadosContratados($mes = '', $omitida = ''){
...
',1,'L',0);
    ...
    public function ResumenNominaDirectivos($mes = '', $omitida = ''){
