...
class PDF_CT_PROTECCION_CIVIL extends \FPDF {
    function Header()
    {
    ...
    $this->SetX(140);
    $this->Cell(80,5,'NOMINA DE: '. 'PROTECCION CIVIL'. ' -  BONO DE ALIMENTACION',0,1,'C');
...
class MRRHHCestaTicketProteccionCivilController extends AppController
{
	public function index($tp = null, $m = null){
        ...
        $tipo_nomina = $this->MrrhhTipoPersonal->find('all', ['conditions' => ['tipo_nomina' => 5, 'id' => 14]])->toArray();
        $options = [];
        ...
        if($tp != null and $m != null ){
            ...
            $query = $this->ViewMrrhhProteccionCivil->find('all');   


            $condicion = ['ano' => ANO_FISCAL, 'mes' => $m, 'id_tipo_personal' => $tp];

            $this->loadModel('ViewMrrhhCestaTicketProteccionCivil');
            $n = $this->ViewMrrhhCestaTicketProteccionCivil->find('all', ['conditions' => $condicion])->count();
            ...
            if($n == 0){
                ...
                foreach ($query as $reg) {
                    ...
                    $this->loadModel('MrrhhCestaTicketProteccionCivil');
                    $ct = $this->MrrhhCestaTicketProteccionCivil->newEntity();
                    $ct = $this->MrrhhCestaTicketProteccionCivil->patchEntity($ct, $this->request->getData());
                    ...
                    if($ct->cedula != 12732280 and $ct->cedula != 14794432){
                        $this->MrrhhCestaTicketProteccionCivil->save($ct);
                    }
                }
            }
        }
        ...
        if($tp != '' and $m != ''){
            ...
            $this->loadModel('ViewMrrhhCestaTicketProteccionCivil');
            $data = $this->paginate($this->ViewMrrhhCestaTicketProteccionCivil->find('all', ['conditions' => $condicion]));
            $this->set(compact('data'));

            $n = $this->ViewMrrhhCestaTicketProteccionCivil->find('all', ['conditions' => $condicion]);
            $this->set('n', $n);
            ...
            $this->loadModel('ViewMrrhhCestaTicketProteccionCivil');
            $db = $this->ViewMrrhhCestaTicketProteccionCivil->find('all', ['conditions' => $condicion]);
            $this->set(compact('db'));

            $ndb = $this->ViewMrrhhCestaTicketProteccionCivil->find('all', ['conditions' => $condicion]);
            $this->set('ndb', $ndb);
    ...
    public function procesar(){
        ...
        if ($this->request->is('post')) {
            ...
            if($this->MrrhhRequisicion->save($req)){
                ...
                $this->loadModel('ViewMrrhhCestaTicketProteccionCivil');
                $query = $this->ViewMrrhhCestaTicketProteccionCivil->query();
                $query->update()
                    ->set(['procesado' => 1])
    ...
    public function eliminar($tp = null, $m = null){
        ...
        if($tp == 14){
            ...
            $this->loadModel('MrrhhCestaTicketProteccionCivil');
            $this->MrrhhCestaTicketProteccionCivil->deleteAll(['id_tipo_personal IN' => $tp, 'ano IN' => ANO_FISCAL, 'mes IN' => $m]);
        }
    ...
    public function eliminarp($id = null, $tp = null, $m = null){
            ...
            $this->loadModel('MrrhhCestaTicketProteccionCivil');
            $this->MrrhhCestaTicketProteccionCivil->deleteAll(['id_tipo_personal IN' => $tp, 'ano IN' => ANO_FISCAL, 'mes IN' => $m, 'id' => $id]);
	...
	public function EditarCT(){
		...
		$query = $this->MrrhhCestaTicketProteccionCivil->updateAll(['dias_trabajados' => $this->request->data['dias_trabajados'], 'dias_faltados' => $this->request->data['dias_faltados'], 'dias_a_cancelar' => $this->request->data['dias_a_cancelar']], ['id' => $this->request->data['id']]);
	...
	public function ReportesProteccionCivil($mes = null){

		$this->loadModel('ViewMrrhhCestaTicketProteccionCivil');
		$condicion = ['ano' => ANO_FISCAL, 'mes' => $mes, 'id_tipo_personal' => 14];
        $data = $this->ViewMrrhhCestaTicketProteccionCivil->find('all', ['conditions' => $condicion]);
        ...
        $pdf = new PDF_CT_PROTECCION_CIVIL('L','mm','LEGAL');
        $pdf -> AddPage();
        
        $pdf -> SetFont('Arial','I',8);
                $pdf -> SetY(20);
                $pdf->SetX(110);
                $pdf -> MultiCell(140,5,'Nomina: PROTECCION CIVIL (Correspondiente al'.' mes de'.' '.$mes['denominacion'].' '.'del'.' '. ANO_FISCAL,0,'C',0);
                $pdf -> SetX(160);
    ...
    public function RequisicionProteccionCivil($mes = '', $requisicion = ''){
...
',1,'L',0);
    ...
    $pdf -> ln();$pdf -> ln();
    $pdf -> SetX(10);
    $pdf -> MultiCell(200,5,'PAGO DE CESTA TICKET DE PROTECCION CIVIL CORRESPONDIENTE AL MES DE'.' '. $mes['denominacion'].' '. 'DEL'. ' '. ANO_FISCAL,0,'L',0);
...
',1,'L',0);
    ...
    public function AcumuladoProteccionCivil($mes = '', $requisicion = ''){
     ...
     $i = 0;
        ...
        $pdf -> Cell(95,4,'Otros complementos al personal empleado',0,0);
...
',1,'L',0);
    ...
    public function txt($tp, $mes, $requisicion){
        ...
        if($tp == 14){
            $this->loadModel('ViewMrrhhCestaTicketProteccionCivil');
            $data = $this->ViewMrrhhCestaTicketProteccionCivil->find('all')->where(['id_tipo_personal' => $tp, 'ano' => ANO_FISCAL, 'mes' => $mes, 'requisicion' => $requisicion]);
        }
